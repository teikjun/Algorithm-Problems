...
public class BFSTopoVersion {
    ...
    /* for each vertex, when the vertex is visited, 
    we store the number of vertices visited so far (including this vertex) as completeCount
    A neighboring vertex n is only visited if completeCount[curr] + 1 > completeCount[n] */
    
    public static void dpAlgo(int s) {
        ...
        while (!queue.isEmpty()) {
            ...
            ArrayList<Integer> neighborList = adjacencyList.get(curr);
            for (Integer n : neighborList) {
                // relax operation
                if (completeCount[curr] + 1 > completeCount[n]) {
                    visited[n] = 1;
                    completeCount[n] = completeCount[curr] + 1;
                    maxCompleteCount = Math.max(maxCompleteCount, completeCount[n]);
                    queue.add(n);
                }
            }    
        }
        // System.out.println("maxCompleteCount: " + maxCompleteCount);
